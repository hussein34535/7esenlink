...
...`;
  ...
  const handleQuickReplace = async () => {
    ...
    if (!confirm(`Replace "${findText}" with "${replaceText}" in ${replaceCategory === 'all' ? 'ALL' : replaceCategory} links?`)) {
      return;
    }
    ...
    try {
      const response = await fetch('/api/links/replace', {
        ...
        body: JSON.stringify({ findText, replaceText, category: replaceCategory }),
      })
    ...
    <div className="container mx-auto py-8 px-4 md:px-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="default">Save Changes <span className="ml-2 text-xs opacity-70">(Auto-saved)</span></Button>
          <Link href="/">
      ...
      <Card className="bg-muted/30">
        <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="space-y-1 w-full md:w-[200px]">
            <label className="text-xs font-medium">In Category</label>
            <Select value={replaceCategory} onValueChange={setReplaceCategory}>
          ...
          <Button onClick={handleQuickReplace} disabled={isActionLoading || !findText}>
      ...
      <div className="flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => loadLinksAndCategories()}
            title="Refresh"
          >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[30%]">Name (Editable)</TableHead>
                      ...
                      <TableRow key={`${link.category}-${link.id}`} className={isSelected ? "bg-muted" : ""}>
                        ...
                        <TableCell className="py-2">
                          <Input
                            ...
                            onBlur={(e) => handleNameBlur(link.id, e.target.value)}
                            className="h-8 border-transparent hover:border-input focus:border-input px-2 -ml-2 font-medium"
                          />
                        </TableCell>
                        <TableCell className="py-2">
                          <Select
                            ...
                            disabled={isActionLoading}
                          >
                            <SelectTrigger className="w-full h-8 text-xs border-transparent hover:border-input">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              {categories.map((category) => (
                        ...
                        <TableCell className="py-2">
                          <div className="flex flex-col gap-1 text-xs">
                            {/* Original URL with Truncate + Copy */}
                            <div className="flex items-center gap-2 group">
                              <span className="font-mono text-muted-foreground w-12 shrink-0">Orig:</span>
                              <div className="relative max-w-[300px] flex-grow">
                                <p className="truncate text-foreground/80 cursor-default" title={link.original}>
                                  {link.original}
                                </p>
                              </div>
                              <Button
                            ...
                            {/* Static URL with Truncate + Copy */}
                            <div className="flex items-center gap-2 group">
                              <span className="font-mono text-muted-foreground w-12 shrink-0">Link:</span>
                              <div className="relative max-w-[300px] flex-grow">
                                <p className="truncate text-blue-600/80 cursor-default" title={fullConvertedUrl}>
                                  {fullConvertedUrl}
                                </p>
                              </div>
                              <Button
                ...
                {editingCategory === cat ? (
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => startEditingCategory(cat)}>
                        <Loader2 className="h-3 w-3" /> {/* Using generic icon for edit if Edit2 isn't imported, but assuming standard Edit icon or text */}
                        <span className="sr-only">Edit</span>
                        ✎
                      </Button>
                      <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => deleteCategory(cat)}>
